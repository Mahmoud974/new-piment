...
exports.modifySauce = (req, res, next) => {
  Sauce.findOne({ _id: req.params.id })
    ...
    .then((sauce) => {
      ...
      // Envoyer un message d' erreur 401
      else {
        res
          .status(401)
          .json({ error: "vous êtes pas autoriser à faire la modification" });
      }
    })
...
exports.deleteSauce = (req, res, next) => {
  Sauce.findOne({ _id: req.params.id }).then((sauce) => {
    ...
    //Comparer l'utilisateur si l'utilisateur obtinet l'autorisation
    if (sauce.userId !== req.auth.userId) {
      res.status(401).json({
        error: new Error("Vous n'etes pas autoriser"),
      });
    }
